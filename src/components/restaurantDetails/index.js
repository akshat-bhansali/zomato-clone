      ...
      render: (k,record) => 

        {
          ...
          <Button
            ...
            type="secondary"
            onClick={() => {
              if(user?.email==null || user?.email == ''){navigate('/login');alert("Login First"); return;}
              addToCart(record);
            }}
          >
            +
    ...
    querySnapshot.forEach(async (doc) => {
      ...
      if(!(data?.resId))
      {
        ...
        if(lis.length==0)
        {
          ...
          await updateDoc(doc.ref, { ...data,cart:lis,resId:null,resImg:null});
          
        }else 
      await updateDoc(doc.ref, { ...data,cart:lis});
      getUserData();
